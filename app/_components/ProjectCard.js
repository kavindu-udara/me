...
const ProjectCard = ({ readme }) => {
    ...
    const navigateToProject = () => {
        window.location.href = `/me/projects/${readme.slug}`
    }

    return (
        <motion.div onClick={navigateToProject} className='rounded-lg backdrop-blur-md bg-gray-200/10 cursor-pointer hover:backdrop-blur-xl' whileHover={{ scale: 1.1 }}
            whileTap={{ scale: 0.95 }} >
